...
| 文件 | 职责 |
| --- | --- |
| `stats.ts` | `StatsService`：三层桶（`globalStats` / `initStats` / `currentRunStats`）累加、`tick()` 帧计数、`PersistedStats` 存档结构、`setRunId` 恢复 |
...
- 帧统计（`framesActive` 等）每帧 `tick()` 累计；资源/物品/剧情统计由写入口同步记账。
