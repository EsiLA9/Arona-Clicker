...
- `npx tsc --noEmit` 零错误；
- `npm test` 全部通过（行为零变化）；
- 若动了 `src/engine/types/`：`npm run gen:schema` 同步协议（见 [[docs-828/05-conventions/schema-sync]]）。
...
[[docs-828/05-conventions/testing]] · [[docs-828/05-conventions/doc-maintenance]]
