...
| `flags` / `initExtras` / `extras` | Record / ExtraCompound | 世界线标记 / per-Init Extra / 全局 Extra（三层见 [[docs-828/02-modules/extra]]） |
...
| `unlockedEnhancements` / `enhancementAttachments` | string[] / Record | 已解锁强化 / 挂靠元数据（仅 UI 展示） |
