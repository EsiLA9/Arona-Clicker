...
- **位置**：`02-modules/stats.md:14,16,22`（三层桶累加、写入口同步记账）；`04-algorithms/roster.md:15`（收集类统计「同步三层统计」义务）；`03-data-structures/player-state.md:35`（`protoStats` 派生视图，Trigger 维护）；`03-data-structures/stats-views.md:5-25`（TagStatService 7 kind × 「声明集 / 收集集 / 实体反查」三套索引 + GameView 快照）。
...
- **方案组**：
  ...
  - C：维持现状。
- **收益注**：A 删掉一条 Trigger 维护链与 7×3 索引重建；写入口从「三件事」减回「两件事」，降低「新增写方法要记得三件事齐全」的纪律负担（对照 [[docs-828/04-algorithms/state-mutation]]）。
...
- **方案组**：
  - **A（推荐）**：删除（纪律 7 下存档结构破坏性变更零成本；玩法立项时重加）。
