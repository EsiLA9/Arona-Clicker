...
- `src/engine/game/per-init-fields.ts` 的 `PER_INIT_FIELD_SPECS` 是 per-Init 字段清单的**唯一登记处**（17 字段）：普通字段 `field()` / Spot 容器 `spotField()` / Character 归属容器 `characterContainer()`。
- `init-savepoint.ts` 的 save / clear / restore 全部遍历 SPECS；`state-factory.createDefaultState` 的 per-Init 部分引用 SPECS。
