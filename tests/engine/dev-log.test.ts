...
describe('DevLog', () => {
  ...
  test('maps production and spending events to useful messages', () => {
    ...
    log.recordEvent({ type: 'spotProduced', spotId: 'printer', resource: 'credit', amount: 5 }, 5);
    log.recordEvent({ type: 'tick', frame: 5 }, 5);
    // 非整 30 帧不产生汇总日志
    log.recordTick({ frame: 5, productions: [{ spotId: 'printer', resource: 'credit', amount: 5 }] });
  ...
  test('summarizes production only on cadence frames', () => {
    const log = new DevLog({ clock: () => 123 });
    // 整 30 帧记录汇总
    log.recordTick({ frame: 30, productions: [{ spotId: 'printer', resource: 'credit', amount: 5 }] });
    expect(log.getEntries()).toMatchObject([
  ...
  test('verbose records every frame, positive income, and per-spot production', () => {
    ...
    // 每条 spot 生产都记录
    log.recordEvent({ type: 'spotProduced', spotId: 'printer', resource: 'credit', amount: 5 }, 7);
    // 非整 30 帧也记录 tick 汇总
    log.recordTick({ frame: 7, productions: [{ spotId: 'printer', resource: 'credit', amount: 5 }] });
  ...
  test('export produces a full JSON snapshot in chronological order', () => {
    ...
    log.recordEvent({ type: 'spotProduced', spotId: 'printer', resource: 'credit', amount: 5 }, 2);
