...
describe('色彩组在测试游戏(baseDatapack)内的解锁与激活链路', () => {
  ...
  test('获得星野 → 自动解锁 hoshino 系全部色彩组(含泳装/墨蓝/黄沙)', () => {
    ...
    for (const id of ['base:colorgroup:abydos-sand', 'base:colorgroup:hoshino-swim', 'base:colorgroup:ink']) {
  ...
  test('获得不同学生各自解锁对应色彩组', () => {
    ...
    expect(game.colorSystem.isGroupOwned(game.state, 'base:colorgroup:rose')).toBe(true);
    game.mutations.acquireCharacter('Serika', 'gacha');
    expect(game.colorSystem.isGroupOwned(game.state, 'base:colorgroup:emerald')).toBe(true);
    game.mutations.acquireCharacter('Yuuka', 'gacha');
    expect(game.colorSystem.isGroupOwned(game.state, 'base:colorgroup:violet')).toBe(true);
    game.mutations.acquireCharacter('Mika', 'gacha');
    expect(game.colorSystem.isGroupOwned(game.state, 'base:colorgroup:amber')).toBe(true);
    game.mutations.acquireCharacter('Iori', 'gacha');
    expect(game.colorSystem.isGroupOwned(game.state, 'base:colorgroup:crimson')).toBe(true);
    game.mutations.acquireCharacter('Miyako', 'gacha');
    expect(game.colorSystem.isGroupOwned(game.state, 'base:colorgroup:teal')).toBe(true);
    game.mutations.acquireCharacter('Saori', 'gacha');
    expect(game.colorSystem.isGroupOwned(game.state, 'base:colorgroup:indigo')).toBe(true);
    game.mutations.acquireCharacter('Arona', 'gacha');
    expect(game.colorSystem.isGroupOwned(game.state, 'base:colorgroup:schale-solid')).toBe(true);
    expect(game.colorSystem.isGroupOwned(game.state, 'base:colorgroup:sky')).toBe(true);
  });

  test('完成欢迎剧情(flag)解锁 flag 系色彩组(粉/青柠/珊瑚)', () => {
    game.mutations.setFlag('momotalk_pink_unlocked', '1');
    for (const id of ['base:colorgroup:momotalk-pink', 'base:colorgroup:lime', 'base:colorgroup:coral']) {
      expect(game.colorSystem.isGroupOwned(game.state, id)).toBe(true);
    }
  });
  ...
  test('激活珊瑚(全量显式 theme) → token 取显式覆盖值', () => {
    game.mutations.setFlag('momotalk_pink_unlocked', '1');
    game.mutations.activateTheme('base:colorgroup:coral');
    const t = game.colorSystem.activeThemeTokens(game.state);
