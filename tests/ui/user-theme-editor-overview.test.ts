...
describe('用户主题编辑器：主题摘要', () => {
  test('摘要显示当前覆盖数量，并提供回到对应编辑分区的目标', () => {
    ...
    expect((html.match(/data-theme-layer-manager-shell/g) ?? [])).toHaveLength(0);
    expect((html.match(/data-theme-layer-editor-dialog/g) ?? [])).toHaveLength(0);
    expect(html).not.toContain('user-theme-global-card');
