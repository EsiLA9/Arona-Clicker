...
describe('用户主题编辑器：图层排序', () => {
  test('按实际视觉层级从上到下显示，而不是把底层显示在顶部', () => {
    ...
    expect(html).not.toContain('data-theme-layer-manager-shell');
  });
  ...
  test('系统层位于用户层之间时，移动按钮仍按完整堆栈启用', () => {
    ...
    const session = {
      ...
      draft: {
        version: 1 as const,
        presentation: { hosts: [{ id: 'global', layers: [layer('bottom', '#111111'), layer('top', '#222222')], layerOrder: ['bottom', 'top', 'system-color-background'] }] },
      },
    };
  ...
  test('Manager 将系统层按完整 order 放在正确的视觉位置，并允许排序', () => {
    ...
    const draft = {
      version: 1 as const,
      presentation: { hosts: [{ id: 'global', layers: [layer('bottom', '#111111'), layer('top', '#222222')], layerOrder: ['bottom', 'system-color-background', 'top'] }] },
    };
