...
export const EVENT_CATALOG: Record<GameEvent['type'], EventCatalogEntry> = {
  ...
  initDefinitionChanged: { purpose: 'Init 内容定义局部热变更', emit: ['runtime-world-content-coordinator'], subscribe: ['init-service', 'visibility', 'trigger-system'] },
  areaDefinitionChanged: { purpose: 'Area 内容定义局部热变更', emit: ['runtime-world-content-coordinator'], subscribe: ['init-service', 'visibility', 'game-num', 'ui-controller-events'] },
  areaTopologyChanged: { purpose: 'Area Runtime 拓扑 Overlay 变化', emit: ['runtime-world-content-coordinator'], subscribe: ['ui-controller-events'] },
  ...
  characterProgressChanged: { purpose: '角色养成变更（伞事件；domain 区分轴）', emit: ['state-mutation-service'], subscribe: ['condition-deps', 'trigger-system', 'ui-controller-events'] },
  ...
  themeChanged: { purpose: '系统 / 色彩组 / 用户自定义主题来源切换', emit: ['state-mutation-service'], subscribe: ['ui-controller'] }, entityThemeChanged: { purpose: '实体主题变化', emit: ['state-mutation-service'], subscribe: ['ui-controller'] }, entityPresentationChanged: { purpose: '实体表现内容选择变化', emit: ['state-mutation-service'], subscribe: ['ui-controller'] }, entityPresentationEffectRequested: { purpose: '实体表现临时覆盖请求', emit: ['effect-engine'], subscribe: ['entity-presentation-runtime'] }, entityDesignUnlocked: { purpose: '实体配色设计解锁', emit: ['state-mutation-service'], subscribe: [] },
