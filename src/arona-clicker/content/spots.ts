...
export const baseSpots: SpotDef[] = [
  spot('base:spot:credit_printer', 'base:area:schale_main').name('信用点制造机').desc('一台老旧但可靠的信用点制造设备，每个 Tick 都会产出信用点。').cost(0).yield(5).capacity(500).managerBonus(3).tags(tagPath('credit'), tagPath('office')).linearYield('base:funclet:credit_printer_linear', Resource.Credit, 2).gacha('base:funclet:credit_printer_gacha').genericUpgrade(50, 2, 2).build(),
  spot('base:spot:comms_terminal', 'base:area:schale_main').name('通讯终端').desc('连接联邦网络的终端设备。可以安全结束当前世界线的经营活动，回到世界线选择。').cost(0).yield(3).capacity(200).managerBonus(1).tags(tagPath('office'), tagPath('tech')).restartInit('base:funclet:comms_restart').genericUpgrade(30, 2, 1).build(),
  spot('base:spot:data_wiper', 'base:area:schale_main').name('数据清除装置').desc('一台带有⚡警示标志的重置设备。可以彻底抹除当前世界线的运营记录，下次进入时为崭新状态。注意：这会删除快照，但会保留所有统计数据。').cost(0).yield(2).capacity(100).managerBonus(1).tags(tagPath('tech'), tagPath('defense')).hardResetInit('base:funclet:data_wiper_hard_reset').genericUpgrade(25, 2, 1).build(),
  spot('base:spot:field_work', 'base:area:schale_main').name('野外调查站').desc('阿比多斯风格的小型户外作业点，适合野外探索型学生。').cost(20).yield(8).capacity(300).managerBonus(4).tags(tagPath('field'), tagPath('combat')).revealResource('name', Resource.Credit, 10).revealResource('utility', Resource.Credit, 40).linearYieldWhen('base:funclet:field_work_conditioned', Resource.Credit, 1, and(cond('stat', '$GlobalProducedAmount base:resource:credit', '>', 100))).levelUpTo(3).genericUpgrade(80, 1.8, 1).build(),
  spot('base:spot:tactical_desk', 'base:area:schale_main').name('战术指挥台').desc('配备通讯设备和地图的指挥站，适合战术型学生。').cost(30).yield(10).capacity(200).managerBonus(5).tags(tagPath('tactical'), tagPath('intel'), tagPath('office')).revealResource('name', Resource.Credit, 20).revealResource('utility', Resource.Credit, 60).levelUpTo(3).genericUpgrade(120, 1.8, 1).build(),
  spot('base:spot:abydos_rehab', 'base:area:abydos_campus').name('对策委员会室').desc('堆满资料与装备的对策室。委员会的日常从这里开始。').cost(0).yield(4).capacity(400).managerBonus(2).tags(tagPath('office'), tagPath('defense')).restartInit('base:funclet:abydos_restart').levelUpTo(3).genericUpgrade(45, 1.8, 1).build(),
  spot('base:spot:abydos_cafe', 'base:area:abydos_campus').name('阿比多斯咖啡角').desc('临时搭起的咖啡角。午后的阳光里，少女们的谈笑声此起彼伏。').cost(15).yield(6).capacity(300).managerBonus(3).tags(tagPath('shop'), tagPath('rest')).revealResource('name', Resource.Credit, 8).revealResource('utility', Resource.Credit, 30).levelUpTo(3).genericUpgrade(70, 1.8, 1).build(),
  spot('base:spot:millennium_lab', 'base:area:millennium_lab').name('千禧年数据分析站').desc('全天候运转的数据分析站。信用点从海量数据流中沉淀出来。').cost(0).yield(7).capacity(350).managerBonus(4).tags(tagPath('tech'), tagPath('intel')).restartInit('base:funclet:millennium_restart').levelUpTo(3).genericUpgrade(60, 1.8, 1).build(),
  ...
  spot('base:spot:trinity_donation', 'base:area:trinity_cathedral').name('圣所捐赠箱').desc('信徒们虔诚的捐赠汇集于此。每一笔捐赠都化为重建学园的基石。').cost(0).yield(6).capacity(500).managerBonus(3).tags(tagPath('faith'), tagPath('courtesy'), tagPath('credit')).restartInit('base:funclet:trinity_restart').levelUpTo(3).genericUpgrade(55, 1.8, 1).build(),
  spot('base:spot:trinity_repair', 'base:area:trinity_cathedral').name('古文书修复台').desc('满是古籍与羊皮纸的修复工坊。每修复一页，信用点便伴随历史苏醒。').cost(30).yield(9).capacity(300).managerBonus(5).tags(tagPath('archive'), tagPath('book')).revealResource('name', Resource.Credit, 15).revealResource('utility', Resource.Credit, 50).levelUpTo(3).genericUpgrade(130, 1.8, 1).build(),
  spot('base:spot:trinity_tea_prep', 'base:area:trinity_tea_room').name('茶会准备台').desc('精致的茶具与点心摆满桌面。淑女们在优雅的茶香中讨论着学园的未来。').cost(25).yield(10).capacity(280).managerBonus(5).tags(tagPath('tea'), tagPath('rest'), tagPath('politics')).revealResource('name', Resource.Credit, 18).revealResource('utility', Resource.Credit, 55).levelUpTo(3).genericUpgrade(110, 1.8, 1).build(),
  spot('base:spot:gehenna_hall', 'base:area:gehenna_council').name('万魔殿会议厅').desc('桌椅歪斜、文件散落的会议厅。看似杂乱无章，却总能高效地完成行政工作。').cost(0).yield(7).capacity(350).managerBonus(4).tags(tagPath('admin'), tagPath('discipline'), tagPath('office')).restartInit('base:funclet:gehenna_restart').levelUpTo(3).genericUpgrade(65, 1.8, 1).build(),
  spot('base:spot:gehenna_bureau', 'base:area:gehenna_district').name('便利屋68据点').desc('挂着"业务中"牌子的小事务所。社长亚瑠的"法外"业务为学园带来意想不到的收益。').cost(35).yield(11).capacity(260).managerBonus(6).tags(tagPath('business'), tagPath('outlaw'), tagPath('shop')).revealResource('name', Resource.Credit, 25).revealResource('utility', Resource.Credit, 70).levelUpTo(3).genericUpgrade(150, 1.8, 1).build(),
];
