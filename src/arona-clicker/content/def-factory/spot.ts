...
export class SpotBuilder {
  ...
  private _levelUpgrades: LevelUpgradeDef[] = []; private _upgradeCostBase?: number; private _upgradeCostGrowth?: number; private _maxLevel?: number;
  ...
  genericUpgrade(upgradeCostBase: number, upgradeCostGrowth: number, linearAmountPerLevel?: number): this {
  ...
  build(): SpotDef {
    ...
    const def: SpotDef = { id: this._id, areaId: this._areaId, name: this._name, description: this._description, baseCost: this._baseCost, baseCostResource: this._baseCostResource, baseCapacity: this._baseCapacity, tags: this._tags };
