...
import type { StatsContext } from '../../engine/contracts/stats';
...
export class StateMutationService implements StateMutationPort, EffectMutationPort {
  ...
  constructor(
    ...
    private progression: CharacterProgressionPort | null = null,
  ) {}

  setProgression(port: CharacterProgressionPort): void {
    this.progression = port;
  }
  ...
  private characterCatalog: {
    getVariant(id: VariantId): import('../types/character').CharacterVariantDef | undefined;
    getCurve(id: string): import('../types/character').CultivateCurveDef | undefined;
  ...
  private curveOf(variant: import('../types/character').CharacterVariantDef) {
    if (!this.progression) throw new Error('StateMutationService progression port is not initialized');
    return this.progression.resolveCurve(variant.curve ? this.characterCatalog?.getCurve(variant.curve) : undefined);
  }
  ...
  addAffectionExp(variantId: VariantId, delta: number): { ok: boolean; newLevel: number; newExp: number } {
    ...
    const cap = this.progression!.affectionLevelCapOf(config, variant, entry.stars);
    const result = this.progression!.applyAffectionExp(config, entry, cap, variant, delta);
  ...
  breakthroughStar(variantId: VariantId): { ok: boolean; reason?: string; newStars?: number } {
    ...
    const check = this.progression!.checkBreakthrough(this.curveOf(variant), state, variant, entry);
