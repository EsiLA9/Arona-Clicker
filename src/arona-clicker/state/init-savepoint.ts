...
export class InitSavepoint {
  constructor(private readonly registry: Registry, private readonly getState: () => AronaClickerState) {}
  ...
  save(initId: string): void {
    ...
    for (const spec of PER_INIT_FIELD_SPECS) spec.capture(this.registry, state, snapshot);
    if (!state.initSnapshots) state.initSnapshots = {};
    state.initSnapshots[initId] = snapshot;
  }
  clear(): void {
    const state = this.state;
    for (const spec of PER_INIT_FIELD_SPECS) spec.clear(this.registry, state);
    state.activeInit = '';
  }
  restore(snapshot: NonNullable<AronaClickerState['initSnapshots']>[string]): void {
    const state = this.state;
    for (const spec of PER_INIT_FIELD_SPECS) spec.restore(this.registry, state, snapshot);
  }
