...
import { hslCss, readableOnColor, toHsl } from '../engine/core/color';
...
function lightBackgroundSurface(
  ...
  const shifted = hslCss(color, { hueShift: hueShiftDeg });
  if (!shifted) return `color-mix(in srgb, ${color} ${percentage}%, transparent)`;
...
export function deriveSurfaceColor(color: string, mode: ThemeMode = 'light', amount = 0.8): string {
  ...
  return hslCss(hsl, { saturationScale: 1 - amount * 0.45, lightness: hsl.l + (0.96 - hsl.l) * amount }) ?? color;
}
...
export function deriveBackgroundGradient(colors: readonly string[], mode: ThemeMode = 'light'): string {
  ...
  const second = colors[1]
    ...
    : lightBackgroundSurface(first, mode, 40);
...
export function resolveThemeNode(
  ...
  if (node === 'bgAlt') return lightBackgroundSurface(selectedColor, 'light', 40);
 ...
 * 文字色判定统一走框架契约（readableOnColor），与 theme-tree 的 --ink-on-*、
 * 背景合成色服务同源——同一背景在 :root / 作用域 / 实体容器里只有一种答案。
 */
export function buildScopedThemeCompatibilityVars(nodes: Readonly<Record<ThemeNodeName, string>>): Record<string, string> {
  const panelInk = readableOnColor(nodes.panel);
  const panelLightInk = readableOnColor(nodes.panelLight);
  const bgInk = readableOnColor(nodes.bg);
  const primaryInk = readableOnColor(nodes.primary);
  const highlightInk = readableOnColor(nodes.highlight);
  const playerBubbleInk = readableOnColor(nodes.playerBubble);
  const npcBubbleInk = readableOnColor(nodes.npcBubble);
  return {
