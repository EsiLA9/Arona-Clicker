...
const TARGET_LEVELS = ['global', 'cluster', 'region', 'control'] as const;
const TARGET_LEVEL_LABELS: Record<typeof TARGET_LEVELS[number], string> = { global: '全局', cluster: '簇', region: '区域', control: '控件' };
...
function renderTargetSummary(ctx: UIContext, presentation: NonNullable<UserThemeDraft['presentation']>): string {
  const configured = presentation.hosts ?? [];
  return configured.length ? renderHostTargets(ctx, presentation, true) : '<p class="modal-empty">暂无已配置的个性化表现目标。可从下方加入目标。</p>';
}
...
function renderHostSummary(ctx: UIContext, host: PresentationHostDef, active: boolean): string {
  ...
  const isGlobal = host.id === 'global';
  const state = isGlobal ? 'default' : (hostStateForRender.get(host.id) ?? 'default');
  const stateDef = state === 'default' ? undefined : host.states?.[state];
  const ref: ThemeLayerTargetRef = isGlobal ? { kind: 'global' } : { kind: 'host', hostId: host.id, state };
