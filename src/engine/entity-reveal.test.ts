...
import { getInitReveal, getAreaReveal, getStoryReveal } from '../ui/components/tooltip';
...
describe('探索机制：Init / Area / Story', () => {
  ...
  test('Story reveal: name hidden until stat threshold; completed is owned', () => {
    expect(getStoryReveal(ctxOf(game), game.registry.stories.get(BRIEFING)!).nameKnown).toBe(false);

    game.mutations.changeResource(CREDIT, 40);
    expect(getStoryReveal(ctxOf(game), game.registry.stories.get(BRIEFING)!).nameKnown).toBe(true);
  });
});
