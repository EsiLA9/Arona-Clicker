...
| 文件                       | 内容                                                         | 典型用途          |
| ------------------------ | ---------------------------------------------------------- | ------------- |
| [[1-项目/ACProgram/docs-818/01-architecture]]      | 系统架构总览、技术栈、设计原则                                            | 理解项目全貌        |
| [[1-项目/ACProgram/docs-818/02-data-structures]]   | PlayerState / Datapack / 实体定义 / 表达式 / 条件 / 效果 / Extra / 事件 | 理解"数据长什么样"    |
...
| [[09-schema-protocol]]   | 实体类型 → 数据包编辑器 的 Schema 描述协议（defMap/defs/extras 拼合 + 同步测试）  | 改实体字段/枚举时查这里  |
| [[10-story-graph]]       | Story 跳转链（goto/insert）/ 条件奖励 / 重阅读 / 分歧守卫                       | 设计剧情图结构时查这里   |
| [[12-character-rework]]  | Character 系统重构设计（实体字段/引擎落点/里程碑）                          | Character 重构时查这里   |
| [[12-character-rework-tests]] | Character 重构测试规格（套件/用例/边界/验收）                              | Character 重构时查这里   |

## 阅读顺序建议

1. **快速了解**：[[00-README]]（本页）→ [[1-项目/ACProgram/docs-818/01-architecture]]
2. **理解数据**：[[1-项目/ACProgram/docs-818/02-data-structures]]
3. **理解机制**：[[03-engine-subsystems]] → [[04-init-lifecycle]]
4. **理解解锁**：[[05-access-stages]]
5. **理解 UI**：[[06-ui-layer]]
6. **理解内容**：[[07-data-pack-system]]
7. **定位修改**：[[08-code-map]]
8. **改实体字段**：[[09-schema-protocol]]
...
| `docs-817/` · `docs-818/` | 设计文档（818 为最新权威版；817 的机制/协议内容已并入 818） |
