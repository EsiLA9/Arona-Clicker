...
- 详见 [[docs/docs-828/04-mechanisms/production]]。
...
- **保留的纪律警告**：`flows` 与 `effects[addResource]` 并存会叠加（激活沿一次 + 每帧持续 = 双倍），数据作者二选一（见 [[docs/docs-828/04-mechanisms/trigger-effect]]「双通道警告」）。
- **范围外未修**：`SpotFunctionalityDef.id` 兼作 pack 命名空间键的隐式覆盖（记录于 [[docs/docs-828/03-data-structures/id-reference-semantics]] §六.3）。

## 相关文档

[[docs/docs-828/02-modules/game-num]] · [[docs/docs-828/02-modules/affector]] · `docs/todoTask/taskProduction/REPORT.md`（执行记录）
