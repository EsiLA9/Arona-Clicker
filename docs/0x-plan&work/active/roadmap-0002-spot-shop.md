...
| Spot 功能项面板 | [[docs/docs-828/02-modules/ui]] | UI 落点：功能项面板扩展商店弹层 |

## 设计方向（草案，待裁定）

- 新增 ShopDef（Datapack 表，按实体归属进入 `src/data-services/contracts/` 或产品内容契约，随后跑 `npm run gen:schema`，协议见 [[docs/docs-828/05-conventions/schema-sync]]）；货架条目 = `{ 商品 ref, 价格, 限购, 可见/可购条件 }`。
...
- 与 [[docs/0x-plan&work/active/roadmap-0001-datapack-management]] 无强依赖，但新表/实体 id 自即日起须符合三段式（S1a 强校验已生效）。
- 独立于 0003/0004，可并行设计。
