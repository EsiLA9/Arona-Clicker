...
| --- | --- | --- | --- | --- | --- |
| D1 | 建立文档职责边界 | 架构、模块、算法、机制、计划之间存在交叉 | `docs/docs-828/` 分区说明 + `docs/0x-plan&work/` 工作入口 | 每类内容只有一个当前事实源 | ✅ 已完成第一轮 |
...
| D5 | 收敛计划与路线图 | active、docs/newPlan、registry&saves 存在交叉和状态漂移 | 更新后的 `docs/0x-plan&work/00-index.md` 与主题入口 | 每个活跃主题有唯一状态入口 | ✅ 第一轮完成 |
...
| `docs/0x-plan&work/active` | 当前 ADR、Roadmap、Task 和状态 | 复制机制正文 |
| `docs/0x-plan&work/completed` | 已完成决策和历史施工记录 | 继续追踪当前实现 |
| `docs/0x-plan&work/docs/newPlan` | 尚未裁定的方案草稿 | 当作已批准机制 |
...
4. 核验 `docs/0x-plan&work` 的 active/docs/newPlan/completed 状态和入口。
...
- `docs/0x-plan&work/00-index` 已增加“已完成但待归档的活动路线”区分，避免把已完成的 roadmap 与未实施方案混为一类。
