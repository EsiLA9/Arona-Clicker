...
| Spot 可声明专有 `gachaPools`，否则全局通用池 | [[docs/docs-828/04-mechanisms/gacha]] | 池间关系（基础/限定）无模型 |
...
4. **`gachaState`（pity/pulls）的键与归属层**：banner 解耦后按 bannerId 记；归属 global 还是 per-Init——与 [[docs/0x-plan&work/active/roadmap-0004-chara-ownership]] 的拥有体系联动裁定。
5. **`refreshWorldPool` 去留**：本目标内接线，还是废弃删除（[[docs/docs-828/07-audit/dormant-machinery]] 已记录为休眠机制）？

## 前置与关联

- **强关联 [[docs/0x-plan&work/active/roadmap-0004-chara-ownership]]**：拥有体系归属翻转影响 drawableOf 排除逻辑与统计口径，建议两目标设计裁定同场进行。
- 依赖 [[docs/0x-plan&work/active/roadmap-0001-datapack-management]] S1c：character/variant id 三段化完成后，池成员引用统一三段 id。
