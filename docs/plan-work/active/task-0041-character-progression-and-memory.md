...
状态：🟢 A 段已实施（2026-09-11）→ 见 [[docs/plan-work/active/adr-0008-character-progression-boundaries]]：C0 已收口；**A 段**（状态与记忆骨架）已落地，**B 段**（投影 / 追赶 / Chara-Spot）推迟另立子任务。本文保留原始设计草案与 §3.2 清理记录。
...
- **事件驱动重同步**：`cultivated` / `affectionChanged` / `characterAcquired` / `equipmentEquipped` / `managerChanged` / 新 `characterProgressChanged` 触发失效，与 [[docs/plan-work/active/task-0034-affector-performance-review]] 的性能结论对齐；
...
2. **roster 默认 scope**：`global`（现状）还是 `init`（roadmap-0004 方向）？Memory/catch-up 只在 init 级下才有意义。**与 [[docs/plan-work/active/roadmap-0004-chara-ownership]] 联合裁定**。
...
- 上游路线：[[docs/plan-work/active/roadmap-0004-chara-ownership]]、[[docs/plan-work/active/roadmap-0003-gacha-pool-model]]、[[docs/plan-work/active/roadmap-0001-datapack-management]]
- 策划草案：[[docs/plan-work/newPlan/01-ownership-and-development]]、[[docs/plan-work/newPlan/05-fragments-and-currency]]、[[docs/plan-work/newPlan/06-meta-loop-and-ui]]、[[docs/plan-work/newPlan/07-mvp-scope]]
- 相邻任务：[[docs/plan-work/active/task-0039-spot-shop-transaction-system]]（培养消费的 Cost 事务语义）、[[docs/plan-work/active/task-0032-passive-story-scheduling]]（Story 消费者）、[[docs/plan-work/active/task-0034-affector-performance-review]]（失效性能）
- 已实现机制：[[docs/plan-work/completed/affection-planning]]、[[docs/plan-work/completed/color-system-plan]]
