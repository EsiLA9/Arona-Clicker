...
B0–B3 已完成并通过类型检查、专项测试、全量测试和构建。B4 已接入剧情临时背景恢复规则、编辑器背景字段映射及 UI 文档；当前剩余编辑器专用可视化预览和更完整的多设备视觉回归。实现采用 ThemeDef.background、按 id 覆盖/匿名追加、独立 DOM 背景容器和 Pic 资源解析。选择页的场景/状态变体另由 [[docs/plan-work/active/task-0025-selector-dynamic-theme]] 管理。
