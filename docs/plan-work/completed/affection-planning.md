...
- **事件更名注记（2026-09-11）**：好感入账事件 `affectionChanged` 已由伞事件 `characterProgressChanged(domain:'affection')` 取代；`RosterEntry` 更名 `VariantProgress`（见 [[docs/plan-work/active/adr-0008-character-progression-boundaries]]）。本文其余机制描述不变。
