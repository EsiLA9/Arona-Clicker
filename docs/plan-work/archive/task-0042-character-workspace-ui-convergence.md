...
状态：done — 已归档（2026-09-15）。原范围已完成，剩余工作为无。
...
- 当前知识已蒸馏至：
  - [[docs/docs-828/02-modules/ui]]（角色 Workspace 三栏宿主与 `panel-tabs-region` / `panel-body` 约定，及 Contacts / Story 接管后的迁移期兼容层说明）
- 设计理由保留于：
  - 本文
- 后续工作：
  - 无
- 准出结论：
  - 原任务范围已完成（角色 Workspace 已收敛到普通游戏态 / 商店态契约）
