...
> 本文回答：如何把用户自定主题编辑器中“目标内展开图层并直接编辑”的模式，收束为“目标总按钮 → 单目标图层管理器 → 单层编辑弹窗”，以及实现前必须遵守的当前代码边界。本文已转为 [[task-0049-user-theme-background-layer-manager]]，保留为方案来源记录，不替代 `src/`、`docs/docs-828/` 或既有 Roadmap 的事实源。

状态：done — 已归档（2026-09-15）。已转为 [[task-0049-user-theme-background-layer-manager]]，后续收口见 [[task-0050-user-theme-layer-overlay-and-global-target-convergence]]。
...
- 当前知识已蒸馏至：
  - [[docs/docs-828/02-modules/ui]]（图层管理器三层结构、全局系统层稳定 ID 与 `BackgroundLayerDef.enabled` 隐藏语义）
- 设计理由保留于：
  - 本文（Inspector → Layer Manager → Layer Editor Dialog 的三层交互目标与非目标）
- 后续工作：
  - [[task-0050-user-theme-layer-overlay-and-global-target-convergence]]（body 级 overlay 架构与 global 双管线收敛）
  - [[task-0051-user-theme-layer-editor-functional-recovery]]、[[task-0052-user-theme-layer-overlay-presentation]]、[[task-0053-user-theme-layer-interaction-fixes]]、[[task-0054-user-theme-layer-type-aware-value-editor]]
- 准出结论：
  - 原任务范围已完成（已转为 Task-0049 及后续任务链）
- 更正（2026-09-15）：
  - 本文 §二「当前事实」列的「当前没有通用的图层隐藏字段」已失效。当前源码 `src/engine/types/theme.ts` 已存在 `BackgroundLayerDef.enabled?: boolean`，隐藏语义已落地。以源码为准，不再改写本文历史正文。
...
- 将评审通过后的实现切片转为 `docs/plan-work/archive/task-*.md` 或独立 ADR，并更新 `docs/plan-work/00-index.md` 状态。
