...
4. **与好感系统的交互**：好感值内嵌 RosterEntry、随 roster 归属层走（[[affection-planning]]）——好感随 Init 重置是否为期望行为？"好感追赶"是否意味着部分好感投入需跨线补偿？

## 前置与关联

- **强关联 [[roadmap-0003-gacha-pool-model]]**：两目标同处角色域、统计口径互相引用，建议同场设计裁定。
- 建议排在 [[roadmap-0001-datapack-management]] S1c 之后实施：先完成 character/variant id 三段化，避免归属翻转与 id 改名叠加。
