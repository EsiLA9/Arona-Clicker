...
> 本文回答：多包管理这一长期目标的切片拆解、当前进度与验收口径。设计权威：[[adr-0004-datapack-management]]（裁定与实现记录，本文不重复设计，只做状态追踪）。
...
S1b 拆分后的 entry id / story id 引用语义权威口径见 [[adr-0004-datapack-management]] §8 表格，此处不复制（防双源漂移）。
