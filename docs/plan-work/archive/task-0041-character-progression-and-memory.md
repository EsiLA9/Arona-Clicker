...
状态：done — 已归档（2026-09-15）。A 段已实施（2026-09-11）→ 见 [[adr-0008-character-progression-boundaries]]：C0 已收口；**A 段**（状态与记忆骨架）已落地，**B 段**（投影 / 追赶 / Chara-Spot）推迟另立子任务。本文保留原始设计草案与 §3.2 清理记录。
...
- **事件驱动重同步**：`cultivated` / `affectionChanged` / `characterAcquired` / `equipmentEquipped` / `managerChanged` / 新 `characterProgressChanged` 触发失效，与 [[task-0034-affector-performance-review]] 的性能结论对齐；
...
2. **roster 默认 scope**：`global`（现状）还是 `init`（roadmap-0004 方向）？Memory/catch-up 只在 init 级下才有意义。**与 [[roadmap-0004-chara-ownership]] 联合裁定**。
...
- 上游路线：[[roadmap-0004-chara-ownership]]、[[roadmap-0003-gacha-pool-model]]、[[roadmap-0001-datapack-management]]
- 策划草案：[[01-ownership-and-development]]、[[05-fragments-and-currency]]、[[06-meta-loop-and-ui]]、[[07-mvp-scope]]
- 相邻任务：[[task-0039-spot-shop-transaction-system]]（培养消费的 Cost 事务语义）、[[task-0032-passive-story-scheduling]]（Story 消费者）、[[task-0034-affector-performance-review]]（失效性能）
- 已实现机制：[[affection-planning]]、[[color-system-plan]]
...
- 当前知识已蒸馏至：
  - [[docs/docs-828/04-mechanisms/gear]]（Gear 三槽与 tier 升级；同源实现见 `src/arona-clicker/services/gear-system.ts`）
  - 角色成长 A 段（状态与记忆骨架）的边界仍由 [[adr-0008-character-progression-boundaries]] 承载，尚未沉淀为 `docs-828` 机制正文
- 设计理由保留于：
  - 本文（Proto / Variant、Current / Memory / Stats、State / Effect 三条边界，与四项主玩法轴）
- 后续工作：
  - [[adr-0008-character-progression-boundaries]]（B 段：统一 Effect 投影、跨世界线追赶、Chara-Spot）
  - [[roadmap-0004-chara-ownership]]（roster scope 与追赶统计联合裁定）
  - [[11-gear-equipment-system]]（tier 效果运行时消费随 B 段）
- 准出结论：
  - 原任务主目标（设计定版 + A 段骨架 + 三代悬空加成链清理）已完成；B 段明确移交 ADR-0008
