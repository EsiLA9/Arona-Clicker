...
状态：done — 已归档（2026-09-15）。事实核验完成；路由与刷新边界已由 [[task-0045-ui-incremental-update-workspace-isolation]] 与 [[task-0047-contacts-story-workspace-ownership]] 落地。
...
- 当前知识已蒸馏至：
  - [[docs/docs-828/02-modules/ui]]（刷新分级：App / Workspace / Region / Behavior；Surface Token 与 stale 更新阻断）
- 设计理由保留于：
  - 本文（刷新分层原则与 4 条对旧基线的修正）
- 后续工作：
  - [[task-0047-contacts-story-workspace-ownership]]（WorkspaceRoute 与稳定 Shell seam 的类型边界）
  - [[task-0067-ui-update-dispatcher-stage2]]（Item / Shop Entry 等 Reveal 缺口的最小 Region 映射）
- 准出结论：
  - 原设计已转化为施工任务并落地主体范围，草案退出 `newPlan/`
...
该类型第一阶段只用于决定刷新所有权，不作为新的持久状态，也不要求马上替换 PanelState。后续若验证稳定，再按 [[task-0040-unified-workspace-frame]] 的 Router 阶段收敛为正式 WorkspaceRoute。
