...
> 本文规划将表现按钮的外部默认包线替换为主题数据驱动的内嵌装饰线。实现时以本文为施工边界；表现宿主的状态合并与编辑器一致性继续遵循 [[docs/docs-828/07-audit/presentation-editor-consistency]]。
>
> 状态：done — 已归档（2026-09-15）。D0–D4 已实施并验证；外部光晕曲度按非目标保留现状。
...
- 当前知识已蒸馏至：
  - [[docs/docs-828/02-modules/ui]]（`background-service` 支持 Host 形状、内嵌装饰线与 hover 层；宿主状态与形状由 `PresentationView` 统一解析）
- 设计理由保留于：
  - 本文
- 后续工作：
  - [[task-0065-ui-presentation-residual-and-visual-regression]]（外部光晕曲度与多设备回归）
- 准出结论：
  - 原任务范围已完成（D0–D4 全部实施）
