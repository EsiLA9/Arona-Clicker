...
状态：active — Contacts / Story 核心迁移完成；通用 Region 扩展与 Character 顶层兼容清理保留为后续收口项

> 本任务把 [[14-contacts-story-workspace-ownership]] 的源码核验与设计草案转为可执行施工任务。它只处理 Workspace 所有权、路由、状态归属、Host 注册和刷新边界，不把通讯录 / 故事的最终内容信息架构伪装成已裁定方案。
>
> 设计输入还包括 [[13-unified-workspace-refresh-boundaries]]（Workspace Router 与刷新域类型边界）。
...
- [[14-contacts-story-workspace-ownership]]
- [[13-unified-workspace-refresh-boundaries]]
...
- [[task-0040-unified-workspace-frame]]：三栏物理骨架与 WorkspaceFrame 契约；
- [[task-0045-ui-incremental-update-workspace-isolation]]：Surface、Host 和增量更新隔离；
- [[task-0042-character-workspace-ui-convergence]]：角色 Workspace 现有内容与视觉接线；
- [[task-0043-topbar-settings-workspace]]：统一 Workspace 入口和返回习惯；
- [[roadmap-0018-ui-host-registry]]：Host Registry 与宿主发现；
- [[roadmap-0020-service-workspaces]]：Records Service / Archive 归属候选。
...
| --- | --- | --- | --- | --- |
| WS-P0 | [[task-0040-unified-workspace-frame]] | F0–F4 已完成主要 Frame / Column 接线 | F5 Router 状态、F6 响应式 / 视觉清理、F7 Host Registry 收尾中与 Contacts / Story 相关的部分 | Contacts / Story 施工前优先收口 |
| WS-P0 | [[task-0045-ui-incremental-update-workspace-isolation]] | P0–P4 已完成 Surface、Dispatcher、Behavior 和 Shop 增量试点 | P7 WorkspaceRoute / Shell seam 评估；P5 Reveal 分类与显式事件映射按新 Workspace 刷新需要前置或并行 | 路由切换和 stale callback 的基础 |
| WS-P0 | [[roadmap-0018-ui-host-registry]] | H0–H2 已完成，H3 / H4 部分完成 | 核对并补齐 Contacts / Story 三栏 Host、父级、serviceId、主题编辑器发现和联合验收 | 新 Workspace Host 的注册基础 |
| WS-P1 | [[task-0042-character-workspace-ui-convergence]] | 已完成角色 Workspace 的 Frame / 视觉收敛 | 不重复实施；直接复用角色内容 renderer 和视觉契约，作为 Contacts selectedVariant 分支基线 | Contacts 角色迁移输入 |
| WS-P1 | [[task-0043-topbar-settings-workspace]] | 已完成顶栏、设置和服务入口统一 | 不重复实施；复用 Workspace route action、返回、主题和服务导航约定 | 新入口与返回语义基线 |
| WS-P2 | [[roadmap-0020-service-workspaces]] | S0 已完成，S1 / S2 部分完成 | 仅在 P0 将 Archive 裁定为 Records Service 时纳入 S4；否则建立显式 Story 子路由 | Archive 的条件依赖 |
...
- [[14-contacts-story-workspace-ownership]]
- [[13-unified-workspace-refresh-boundaries]]
- [[task-0040-unified-workspace-frame]]
- [[task-0045-ui-incremental-update-workspace-isolation]]
- [[task-0042-character-workspace-ui-convergence]]
- [[task-0043-topbar-settings-workspace]]
- [[roadmap-0018-ui-host-registry]]
