...
状态：closing — 首轮代码施工完成，但浮窗架构验收未通过；剩余收口转入 [[task-0050-user-theme-layer-overlay-and-global-target-convergence]]

> 本任务将 [[15-user-theme-background-layer-manager]] 的方案草案转为可执行施工任务。目标是把用户主题 Inspector 中的图层内联展开编辑，改造成“目标总按钮 → 单目标 Layer Manager → 单层 Layer Editor Dialog”。具体主题解析、背景安全约束和 UI 刷新事实仍以源码与 `docs/docs-828/` 为准。
...
- [[15-user-theme-background-layer-manager]]：已核查的设计草案；
- [[task-0048-workspace-theme-restructure-and-editor]]：当前 Workspace Theme 编辑器与来源模型收口；
- [[roadmap-0011-ui-component-layer-backgrounds]]：控件背景图层目标；
- [[roadmap-0012-flat-presentation-targets]]：平级表现目标与 Host 注册；
- [[roadmap-0013-presentation-editor-ux]]：统一图层字段与局部刷新基础；
- [[roadmap-0014-theme-editor-convergence]]：主题编辑器信息架构与可理解性；
- [[adr-0006-ui-background-layering]]：背景层类型、排序、安全与系统颜色层；
...
- [x] 已阅读 `docs/docs-828/00-INDEX`、`docs/plan-work/00-index` 与 `docs/plan-work/archive/00-index`；
...
- [x] 复核发现首版 Manager/Dialog 的挂载位置仍错误：`renderTargetSummary()` 将 shell 拼入 Inspector，Controller 从父级 modal 查询 shell，不能视为已完成的浮窗验收；详见 [[task-0050-user-theme-layer-overlay-and-global-target-convergence]]。
...
- [[15-user-theme-background-layer-manager]]
