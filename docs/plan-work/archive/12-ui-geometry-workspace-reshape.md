...
状态：done — 已归档（2026-09-15）。已转 Task-0044 并实施完成；本文件保留为上游方案与未裁定项记录。

上游：用户提供的《Task 004x：UI Geometry Contract 与 Workspace 视觉基础设施重塑》

本地基线：[[task-0040-unified-workspace-frame]]、[[task-0042-character-workspace-ui-convergence]]、[[task-0043-topbar-settings-workspace]]、[[10-inventory-workspace]]、[[docs/docs-828/02-modules/ui]]
...
- 当前知识已蒸馏至：
  - [[docs/docs-828/02-modules/ui]]（`WorkspaceFrame` / `WorkspaceColumn` 的 surface、responsive profile 与 `data-workspace-*` 契约；`panel-tabs-region`；scroll owner）
- 设计理由保留于：
  - 本文（几何职责分层与几何契约思想）
- 后续工作：
  - [[roadmap-0018-ui-host-registry]]、[[roadmap-0020-service-workspaces]]（服务宿主发现与服务工作区推进）
  - 明确裁定不实现：`visual-heavy` preset、`VisualStage`、`WorkspaceRouter`（见 completed/task-0044 P4）
- 准出结论：
  - 原任务范围已完成（已转 Task-0044 并实施完成）
...
- 将完成后的实现切片移入 `active/` 或 `completed/`，同步 `docs/plan-work/00-index.md`。
