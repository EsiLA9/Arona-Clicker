...
状态：done — 已归档（2026-09-15）。设计已裁定，施工归入 [[task-0032-passive-story-scheduling]]。
...
- 当前知识已蒸馏至：
  - [[docs/docs-828/02-modules/story]]（Story 服务与被动闲聊基线）
  - [[docs/docs-828/03-data-structures/player-state]]（`studentBlocks` / `passiveCooldowns` 当前仍在 PlayerState 顶层）
- 设计理由保留于：
  - 本文（StoryDef / StoryChain / PassiveEntry / Pool / StoryRun 分层与逐条审阅）
- 后续工作：
  - [[task-0032-passive-story-scheduling]]（`completionKey`、`requestStoryStart` / Eligibility、hierarchical 池与 per-Init 迁移）
- 准出结论：
  - 原任务范围已完成（设计意见已完成逐条审阅与裁定，转入施工任务）
...
用户裁定执行 P0、P1，并将 Pool 分层权重从原 P2 提前纳入当前任务。具体施工范围与验收标准见 [[task-0032-passive-story-scheduling]]。
...
- 当前好感/尾巴方案：docs/plan-work/archive/affection-planning.md
