...
状态：done — 已归档（2026-09-15）。首版已实施；剩余能力已拆至 [[task-0068-inventory-workspace-remaining-capabilities]]。
...
- 当前知识已蒸馏至：
  - [[docs/docs-828/02-modules/ui]]（背包作为独立 `WorkspaceFrame` 服务页与顶栏一级入口）
  - [[docs/docs-828/03-data-structures/player-state]]（`inventory` 为 per-Init 字段）
- 设计理由保留于：
  - 本文（筛选 / 排序 / 自定义顺序与游戏状态隔离的边界）
- 后续工作：
  - [[task-0068-inventory-workspace-remaining-capabilities]]（整理持久化、出售 / 丢弃、来源筛选与体验评审）
- 准出结论：
  - 原任务首版范围已完成（三栏 Workspace、筛选、排序与使用物品链路均已落地）；剩余能力已转独立 Task
