...
状态：done — 已归档（2026-09-15）。P0–P2 与 Shop 三栏 Workspace 移植均已完成；本草案剩余设计裁定归入 [[task-0039-spot-shop-transaction-system]] 与 [[roadmap-0002-spot-shop]]。
...
- 当前知识已蒸馏至：
  - [[docs/docs-828/04-mechanisms/state-mutation]]（`commitShopTransaction` 原子提交）
  - [[docs/docs-828/04-mechanisms/trigger-effect]]（`shopPurchased` 事件）
  - [[docs/docs-828/02-modules/ui]]（Shop 作为独立 `WorkspaceFrame` 服务页）
- 设计理由保留于：
  - 本文（四条冻结边界与玩法定位）
  - [[adr-0007-shop-transaction-boundaries]]（购买记录、原子提交与事件边界裁定）
- 后续工作：
  - [[task-0039-spot-shop-transaction-system]]（P3 验收）
  - [[roadmap-0002-spot-shop]]（完整 `ShopOffer` union、`refresh` 库存类型、实例物品货币）
- 准出结论：
  - 原任务范围已完成（P0–P2 与三栏 Workspace 移植均已落地）
- 更正（2026-09-15）：
  - 本文文首状态与 §11（2026-09-10）记录的「当前把 `ShopSession` 放入 app-modal / 待执行三栏移植」已失效。当前源码中 `src/ui/components/shop.ts` 已输出三栏 `ShopWorkspaceView`，`src/ui/controller-modals.ts` 的 `openSpotShopModal` 实际进入 `enterWorkspaceRoute({ kind: 'shop' })`；该函数名仅为 legacy 命名。以源码为准，不再改写本文历史正文。
...
- [[roadmap-0002-spot-shop]]：Spot 商店目标与当前状态
- [[03-spot-shop]]：早期 Spot 商店草案
