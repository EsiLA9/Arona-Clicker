...
>
> 状态：active — P0 首轮诊断、P1 摘要卡和 P2 Workspace owner 首版接线已完成；来源链 UI、目标目录和全生命周期回归仍在推进。本文记录范围、施工切片、验收口径和后续 ADR 门槛，不替代 `docs/docs-828/` 机制正文。
>
...
- [[roadmap-0014-theme-editor-convergence]]：主题编辑器信息架构与来源可理解性。
- [[roadmap-0016-cluster-region-context-overrides]]：簇/区域上下文覆盖与三栏宿主关系。
- [[roadmap-0017-theme-state-and-semantic-storage]]：状态表现、语义色存储和运行时解析结果。
- [[task-0022-theme-definition-and-custom-theme-repair]]：主题来源、默认主题和自定义主题问题清单。
- [[task-0040-unified-workspace-frame]]：统一三栏 Workspace 物理骨架。
- [[task-0045-ui-incremental-update-workspace-isolation]]：Workspace 刷新边界与隔离。
