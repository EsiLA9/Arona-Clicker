...
> 本文回答：**招募（gacha）怎么结算。** `GachaService.roll(poolId, count)`；招募入口在 Spot 的 `gacha` 功能项（不在通讯录，见 [[docs/docs-828/02-modules/character]]）。
...
- `refreshWorldPool`（池关闭成员并入世界 Pool）已实现但无调用点（见 [[docs/docs-828/03-data-structures/id-reference-semantics]] 特别说明 ③）。

## 相关文档

[[docs/docs-828/03-data-structures/character-entities]] · [[docs/docs-828/04-mechanisms/cultivate]]
