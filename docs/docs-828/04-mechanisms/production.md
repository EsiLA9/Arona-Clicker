...
> 本文回答：**每帧产出怎么算。** 生产 = 数值树求值 + zone 聚合；模块卡片见 [[docs/docs-828/02-modules/game-num]]；设计决策见 [[adr-0002-gamenum-tree]]。
...
- Affector 的 `zoneModifiers` 由 `syncAffectorZoneEffects` 在挂载/翻转/世界线切换时同步进区表（**不再每 tick 全量重建**，见 [[docs/docs-828/04-mechanisms/trigger-effect]]）。
...
[[docs/docs-828/02-modules/game-num]] · [[adr-0002-gamenum-tree]] · [[docs/docs-828/02-modules/affector]]
