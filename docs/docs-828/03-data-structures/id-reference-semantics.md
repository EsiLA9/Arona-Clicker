...
| `TriggerEventDef` on:{…}（9 种 kind，见 [[docs/docs-828/03-data-structures/declarative-dsl]] §5） | 各实体 | 意义 | 事件载荷 id 相等比较（trigger-system）；载荷 id 来自 StateMutationService 运行时快照，完全不读 Def 内容 |
...
| `PassiveStoryEntry.pushAfterStory` | StoryDef（演出本体 id） | 真 | 羁绊尾巴挂靠：关联剧情完结后强制优先推送进 owner 对话空间（见 [[docs/0x-plan&work/completed/affection-planning]] §3） |
...
| 引用字段 | 指向 | 分类 | 实际效果 |
| --- | --- | --- | --- |
| `FuncletCall.funcletId` / Value source `funclet` | FuncletDef | 真 | 取 def.calc 求值（`expression/value-system.ts:79` 经 `funclet-executor.ts`）。⚠️ calc 求值有已知缺陷恒返回 0，见 [[docs/docs-828/03-data-structures/declarative-dsl]] §2 |
...
[[docs/docs-828/03-data-structures/declarative-dsl]] · [[docs/docs-828/05-conventions/architecture-discipline]]
