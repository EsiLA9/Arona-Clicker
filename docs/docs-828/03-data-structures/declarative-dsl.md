...
`TriggerEventDef.kind`（`types/trigger.ts:63-74`，**9 种**）—— Trigger「何时检查」，均可带可选过滤 id；kind 全集与 `TriggerSystem.ON_KIND_TO_EVENT` 双向锁合、且登记于 `EVENT_CATALOG`（[[adr-0001-architecture-consolidation]] T4）：
