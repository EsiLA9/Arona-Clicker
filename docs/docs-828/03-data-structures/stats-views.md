...
> 本文回答：**统计桶怎么分层、统计 DSL 怎么写、UI 视图长什么样。** 模块卡片见 [[docs/docs-828/02-modules/stats]] 与 [[docs/docs-828/02-modules/visibility]]。
...
- 计算依据：`revealTriggers` 的 `existence` 目标（reveal 部分见 [[docs/docs-828/04-mechanisms/trigger-effect]]）；
- 注册表加载使用 `rebuild()`，读档使用 `recomputeAll()`；运行时事件通常只标脏并在读取时增量重算。注意 `RevealStage`（7 级）与 `AccessStage`（5 阶段）是两套概念，勿混淆（见 [[docs/docs-828/02-modules/visibility]]）。

## 相关文档

[[docs/docs-828/03-data-structures/player-state]] · [[docs/docs-828/02-modules/ui]]
