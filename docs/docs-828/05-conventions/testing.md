...
| `tests/engine/` | 基础引擎与 AronaClicker Runtime/领域服务测试（按机制或领域文件命名；当前不再使用独立的 `tests/engine/game/` 子目录） |
...
- **不留迁移测试**：不做存档迁移（纪律 8），旧结构测试直接改断言。

## 相关文档

[[docs/docs-828/05-conventions/architecture-discipline]] · [[docs/docs-828/05-conventions/refactoring]]
