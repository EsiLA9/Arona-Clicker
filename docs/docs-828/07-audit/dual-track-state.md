...
> 引用语义的权威评审在 [[docs/docs-828/03-data-structures/id-reference-semantics]] §六，本篇不重复其结论，只补方案组。
...
- **方案组**：
  ...
  - B：保留现状（编译期成本已付、运行时无成本），仅把「新增字段流程」压缩为一页 checklist（见 [[docs/docs-828/07-audit/sync-burden]] #2）。
