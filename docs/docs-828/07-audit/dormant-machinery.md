...
| `chatRead` / `markChatRead` / `chatReadChanged` | `03-data-structures/player-state.md`、`04-mechanisms/trigger-effect.md`、[[affection-planning]] | 保留、无写入方（裁定供复用） | 限期接线或删 |
...
| 组件级 CSS 变量预留覆写点 | `02-modules/ui.md`、[[affection-planning]] §3 | 预留、有明确重肤用途 | 保留（低优先） |
