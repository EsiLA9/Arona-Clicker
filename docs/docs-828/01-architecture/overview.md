...
> 时序细节见 [[docs/docs-828/01-architecture/run-logic]]；模块定位见 [[docs/docs-828/00-INDEX]] 的模块卡片索引。
...
| 单一写入口 | 所有状态变更经 `StateMutationService`，禁止绕过直改 `PlayerState` | [[docs/docs-828/04-mechanisms/state-mutation]] |
| 事件驱动 | 写状态后广播事件，联动逻辑做成 Trigger / Affector 订阅响应 | [[docs/docs-828/04-mechanisms/trigger-effect]] |
| 只读 UI | UI 只消费 `getView()` / `createUIContext()` 的不可变快照，组件类型面使用 `GameReadModel` | [[docs/docs-828/02-modules/ui]] |
| 三层状态 | 跨世界线（global）/ per-Init 快照 / per-Init 当前 | [[docs/docs-828/01-architecture/state-layers]] |
...
完整时序见 [[docs/docs-828/01-architecture/run-logic]]。
...
- 命令见 [[docs/docs-828/00-INDEX]] 命令速查。
- 存档走 localStorage（`src/data-services/persistence/storage.ts`）；**项目不做存档迁移**（结构可破坏性变更，旧档直接清）。

## 下一步

- 建立运行心智模型 → [[docs/docs-828/01-architecture/run-logic]]
- 理解数据怎么放 → [[docs/docs-828/01-architecture/state-layers]] + [[docs/docs-828/01-architecture/data-flow]]
