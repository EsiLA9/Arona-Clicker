...
> 各层细节：Registry [[docs/docs-828/03-data-structures/registry]]、PlayerState [[docs/docs-828/03-data-structures/player-state]]、统计与视图 [[docs/docs-828/03-data-structures/stats-views]]。
...
- 加载期引用校验（`registry-validate.ts` + `validateCharacterRefs`）：失败即抛错，保证运行时「拿到的引用必有效」（覆盖面见 [[docs/docs-828/03-data-structures/id-reference-semantics]]）。
...
- 三层结构见 [[docs/docs-828/01-architecture/state-layers]]；唯一写入口 `StateMutationService`。
...
| 派生物 | 持有者 | 失效方式 |
| --- | --- | --- |
| GameNum 产出树 | `GameNumSystem` | 事件定向失效（见 [[docs/docs-828/04-mechanisms/production]]） |
