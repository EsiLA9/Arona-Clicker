...
init(datapacks)
 ...
 2. 建产出树：gameNumSystem.buildAll()（四级层级树，见 [[docs/docs-828/04-mechanisms/production]]）
...
- **失效策略（事件驱动）**：tick 不再每帧失效产出树——状态变更经 `StateMutationService` 发事件定向失效（`resourceChanged` 三路定向）；未受影响子树跨帧保持缓存。绕过写入口直改 state 会得到陈旧读数。详见 [[docs/docs-828/04-mechanisms/production]]。
...
下一步读 [[docs/docs-828/01-architecture/state-layers]]（运行时持有的 PlayerState 长什么样）。
