...
- **管**：剧情启动/推进/选择/跳转链/重读守卫/完结奖励、聊天流演出事件、被动闲聊抽选、好感台阶/羁绊尾巴的就绪队列推送（见 [[affection-planning]]）。
...
- **两实体分离**：`StoryEntryDef`（active/passive 分表：触发条件/奖励/可用性）与 `StoryDef`（talklets 演出内容）；`entry.storyId` 是真引用，当前约定 `entry.id === storyId`（1:1，解耦风险见 [[docs/docs-828/03-data-structures/id-reference-semantics]]）。
...
[[docs/docs-828/03-data-structures/declarative-dsl]]（Talklet/Entry 枚举）· [[affection-planning]]（好感数值/台阶推送/羁绊尾巴）
