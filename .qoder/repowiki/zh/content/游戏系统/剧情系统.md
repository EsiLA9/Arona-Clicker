...
- 实现羁绊尾巴（pushAfterStory）管理机制，处理羁绊剧情的后续流程
...
- [story-flow.ts:1-501](file://src/engine/game/story-flow.ts#L1-L501)
...
- story-flow：剧情启动/推进/发送主流程，包含 beginStory、startStory、triggerPassiveStory、triggerAffectionPush、triggerTailPush、readyStepIds、advanceStory、clickSend、getSendState、getCurrentStoryView、trackFlagsAndApply、applyStoryTravel。
...
**章节来源**
- [story-service.ts:52-298](file://src/engine/game/story-service.ts#L52-L298)
- [story-cursor-state.ts:1-96](file://src/engine/game/story-cursor-state.ts#L1-L96)
- [story-flow.ts:26-501](file://src/engine/game/story-flow.ts#L26-L501)
...
+triggerTailPush(owner, storyId)
+readyStepCount(owner)
...
- 职责：
  ...
  - 对外 API：startActiveStory、startCardStory、startStory、triggerPassiveStory、triggerAffectionPush、triggerTailPush、readyStepCount、replayStory、advanceStory、getSendState、clickSend、getCurrentStoryView。
- 关键设计：
  ...
  - readyStepCount 提供就绪队列条数统计，用于未读计数显示。
...
- [story-service.ts:228-230](file://src/engine/game/story-service.ts#L228-L230)
...
- **推送机制**：
  ...
  - triggerTailPush：关联剧情完结后立即定向推送对应尾巴
  - readyStepCount：提供就绪队列条数统计
  - 队列空时回落至普通闲聊随机抽取
...
### 羁绊尾巴管理系统
**新增功能**：羁绊尾巴（pushAfterStory）管理机制，确保羁绊剧情的完整体验流程。

- **尾巴登记**：
  - triggerTailPush：关联剧情完结后定向推送对应尾巴
  - 无尾巴的消息：立即标记为已读
  - 有尾巴的消息：延迟到尾巴完成后标记
...
UI->>SS : triggerTailPush(owner, storyId)
SS->>SF : triggerTailPush(runtime, owner, storyId)
Note over SS,SF : 验证尾巴资格并启动
loop 羁绊尾巴播放
UI->>SS : advanceStory(...)
SS->>SF : 推进剧情
end
Note over SS : 自动清理尾巴状态
```

**图表来源**
- [story-service.ts:217-222](file://src/engine/game/story-service.ts#L217-L222)
- [story-flow.ts:204-217](file://src/engine/game/story-flow.ts#L204-L217)
...
### 就绪队列算法详解
**新增功能**：就绪队列算法的实现细节，包括资格判定和排序逻辑。

- **资格判定函数 isReadyStep**：
  - 排除可重复条目（repeatable）
  - 检查 owner 壁垒和阻断状态
  - 排除正在播出的条目
  - 区分尾巴（pushAfterStory）和台阶（affectionRequired）
  - 验证关联剧情完成状态或好感度要求
  - 检查世界线、触发条件和冷却时间
...
**章节来源**
- [story-flow.ts:477-501](file://src/engine/game/story-flow.ts#L477-L501)
- [chat-flow-service.ts:15-37](file://src/engine/game/chat-flow-service.ts#L15-L37)
...
- **新增**：就绪队列预计算：readyStepIds 函数预先计算就绪队列，避免重复筛选。
- **新增**：尾巴抑制机制：pushAfterStory 检查避免重复触发同一羁绊消息。
- **新增**：队列排序优化：尾巴和台阶分类处理，减少不必要的比较操作。
...
- 排查步骤：
  ...
  - **新增**：检查就绪队列状态（NoAvailableStory 可能因队列空）。
  - **新增**：检查尾巴状态（pushAfterStory 关联剧情未完成）。
...
- StoryService 公开方法
  ...
  - **新增**：triggerTailPush(owner, storyId): StoryStartResult
  - **新增**：readyStepCount(owner): number
...
- 参数与返回值说明
  ...
  - **新增**：storyId（triggerTailPush）：羁绊尾巴关联的演出 Story ID。
- 错误码
  - 见"故障排查指南"中的错误码与含义。
