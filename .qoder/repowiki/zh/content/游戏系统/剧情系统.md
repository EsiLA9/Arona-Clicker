...
- [character-rework.ts](file://src/data/base/character-rework.ts)
</cite>

## 更新摘要
**变更内容**
- 新增情感驱动故事队列系统，支持按好感度等级推送剧情
- 实现羁绊尾巴（pendingKizunaTails）管理机制，处理羁绊剧情的后续流程
- 增强故事优先级系统，情感故事优先于普通闲聊
- 扩展 PassiveStoryEntry 类型，支持 affectionRequired 和 pushAfterStory 字段
...
- **新增**：情感驱动故事队列系统，支持按好感度等级和羁绊关系智能推送剧情。
- **新增**：羁绊尾巴管理机制，确保羁绊剧情的完整体验流程。
...
**章节来源**
- [story-service.ts:1-319](file://src/engine/game/story-service.ts#L1-L319)
- [story-flow.ts:1-498](file://src/engine/game/story-flow.ts#L1-L498)
...
**章节来源**
- [story-service.ts:52-319](file://src/engine/game/story-service.ts#L52-L319)
- [story-cursor-state.ts:1-96](file://src/engine/game/story-cursor-state.ts#L1-L96)
- [story-flow.ts:26-498](file://src/engine/game/story-flow.ts#L26-L498)
...
-pendingKizunaTails : Map<string, string>
...
+triggerAffectionPush(owner)
+startMessageKizuna(messageId, owner)
+getPendingKizunaTail(owner)
+completeKizunaTail(owner)
...
**图表来源**
- [story-service.ts:52-92](file://src/engine/game/story-service.ts#L52-L92)
...
- 职责：
  ...
  - **新增**：羁绊尾巴管理：pendingKizunaTails 映射跟踪待收尾的羁绊消息。
  - 移动规则：isBlockingMovement、clearPassiveIfPlaying。
  - 对外 API：startActiveStory、startCardStory、startStory、triggerPassiveStory、triggerAffectionPush、startMessageKizuna、getPendingKizunaTail、completeKizunaTail、replayStory、advanceStory、getSendState、clickSend、getCurrentStoryView。
- 关键设计：
  ...
  - **新增**：pendingKizunaTails 存储角色到羁绊消息ID的映射，用于追踪待处理的羁绊尾巴。
...
- [story-service.ts:217-251](file://src/engine/game/story-service.ts#L217-L251)
...
### 情感驱动故事队列系统
**新增功能**：情感驱动的故事队列系统，支持按好感度等级和羁绊关系智能推送剧情。

- **就绪队列资格判定**：
  - 尾巴（pushAfterStory）：关联剧情已完结 ∧ 本条未播过
  - 台阶（affectionRequired）：好感度达标 ∧ 本条未播过
...
- **推送机制**：
  - triggerAffectionPush：进入对话空间时自动推送队列顶剧情
  - clickSend idle：点击回复按钮时优先尝试推送队列顶
  - 队列空时回落至普通闲聊随机抽取

```mermaid
flowchart TD
Start(["触发情感推送"]) --> CheckQueue{"就绪队列是否为空?"}
CheckQueue --> |否| PickTop["选择队列顶条目"]
PickTop --> Validate["验证资格<br/>owner/冷却/条件"]
Validate --> StartStory["启动剧情"]
CheckQueue --> |是| RandomChatter["随机抽取普通闲聊"]
RandomChatter --> NoAvailable{"找到闲聊?"}
NoAvailable --> |是| StartStory
NoAvailable --> |否| ReturnError["返回 NoAvailableStory"]
```

**图表来源**
- [story-flow.ts:135-199](file://src/engine/game/story-flow.ts#L135-L199)
- [story-flow.ts:398-407](file://src/engine/game/story-flow.ts#L398-L407)
...
### 羁绊尾巴管理系统
**新增功能**：羁绊尾巴（pendingKizunaTails）管理机制，确保羁绊剧情的完整体验流程。

- **尾巴登记**：
  - startMessageKizuna：点击羁绊消息卡片时登记 pendingKizunaTails
  - 无尾巴的消息：立即标记为已读
  - 有尾巴的消息：延迟到尾巴完成后标记
...
- **尾巴收尾**：
  - completeKizunaTail：UI 调用完成尾巴展示
  - 标记聊天消息为已读
  - 清除 pending 状态，恢复情感推送
...
UI->>SS : startMessageKizuna(messageId, owner)
SS->>SS : pendingKizunaTails.set(owner, messageId)
SS->>SF : startCardStory(kizunaStoryId, owner)
Note over SS,SF : 登记尾巴，开始羁绊剧情
loop 羁绊剧情播放
UI->>SS : advanceStory(...)
SS->>SF : 推进剧情
end
UI->>SS : completeKizunaTail(owner)
SS->>SS : pendingKizunaTails.delete(owner)
SS->>SS : mutations.markChatRead(messageId)
Note over SS : 恢复情感推送
```

**图表来源**
- [story-service.ts:226-251](file://src/engine/game/story-service.ts#L226-L251)
- [story-flow.ts:205-214](file://src/engine/game/story-flow.ts#L205-L214)
...
- 发送 clickSend：
  ...
  - **新增**：优先尝试情感推送（triggerAffectionPush），成功后直接返回；失败后继续普通闲聊抽取。
...
SF->>SF : triggerAffectionPush(owner)
alt 情感推送成功
SF-->>UI : idle(started=true)
else 情感推送失败
...
- [story-flow.ts:368-440](file://src/engine/game/story-flow.ts#L368-L440)
- [story-jump.ts:17-42](file://src/engine/game/story-jump.ts#L17-L42)
...
**章节来源**
- [story-flow.ts:474-498](file://src/engine/game/story-flow.ts#L474-L498)
- [chat-flow-service.ts:15-37](file://src/engine/game/chat-flow-service.ts#L15-L37)
...
- **新增**：情感队列预计算：readyStepIds 函数预先计算就绪队列，避免重复筛选。
- **新增**：尾巴抑制机制：pendingKizunaTails 检查避免重复触发同一羁绊消息。
...
- 排查步骤：
  ...
  - **新增**：检查情感队列状态（NoAvailableStory 可能因队列空或尾巴抑制）。
  - **新增**：检查羁绊尾巴状态（pendingKizunaTails 可能导致推送被抑制）。
...
## 结论
ACProgram 的剧情系统以 StoryService 为核心，通过 StoryRuntime 将流程控制、跳转链、重阅读、奖励结算等子模块解耦，配合 StoryCursorState 的全局/聊天沙盒双游标机制，实现了并行互不打断的对话空间。**新增的情感驱动故事队列系统和羁绊尾巴管理机制**进一步增强了剧情的智能化和连贯性，确保重要剧情能够按照预设逻辑有序推送，同时保持良好的用户体验。Talklet 构建器与 UI 渲染管线提供了灵活的对话表达与交互体验。存档机制确保游标与聊天历史的可恢复性。整体设计兼顾可扩展性、可维护性与性能。
...
- StoryService 公开方法
  ...
  - **新增**：triggerAffectionPush(owner): StoryStartResult
  - **新增**：startMessageKizuna(messageId, owner): StoryStartResult
  - **新增**：getPendingKizunaTail(owner): string | null
  - **新增**：completeKizunaTail(owner): void
