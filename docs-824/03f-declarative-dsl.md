# docs-824 — 03f 声明式 DSL 枚举目录 ==new==

> 本文集中编目引擎里所有**枚举式 / tagged-union** 设计（判别字段 `type` / `kind` / `source` / `t` / `op` / `target` / `category` 等），供数据作者与改引擎者一屏查全。权威定义在 `src/engine/types/`；改这些字段/枚举后必须 `npm run gen:schema`（见 AGENTS.md 同步协议）。
> 运行时事件 `GameEvent` 的枚举目录见 [[docs-824/04f-trigger-effect]]「GameEvent 事件目录」；GameNum 数值树节点 kind 见 [[docs-824/04b-production]]；ZoneModifier category 见 [[docs-824/04f-trigger-effect]]。本文不重复，只补其余 DSL 枚举。
...
> 注意分工：`ValueExpression` 是**数据包声明语言**（含 div/min/max/pow/clamp/floor/ceil/round）；GameNum 数值树是**运行时结算树**，Phase 3 已删去其 9 个未构造 kind，通用算术一律经 `expr` 节点下沉到 `ValueExpression`。

## 2. Funclet（可复用数值片段）

`FuncletDef`（`types/expression.ts`）：`{ id, description, params: {name, type:'number'|'string'}[], calc: ValueExpression, extra? }`；`FuncletCall`：`{ funcletId, args }`。`params[].type` 二选一：`number` / `string`。经 `ValueSource='funclet'` 在任意表达式中调用。

> ⚠️ **已知缺陷（未修，独立任务）**：`value-system.ts:122` 把 `def.calc`（`ValueExpression`）强转成 `Value` 塞进 `{type:'value', value: calc}`，`evaluate` 递归到 value 节点读 `.source`（undefined）落 `default → 0`——**funclet 的 `calc` 运行时恒返回 0**。正确应为 `this.evaluate(def.calc, state)`。见 `todoTask/taskProduction/HANDOFF.md` §5。

## 3. 条件（ConditionSystem）

```
Comparator（types/expression.ts）：== != >= <= > <。

Condition：{ target: ConditionTarget, key, comparator, value }；ConditionGroup：{ type: 'AND' | 'OR', conditions: (Condition|ConditionGroup)[] }（可嵌套）。

ConditionTarget（types/expression.ts，15 种）—— key 与 actual 语义：
```
...
`EffectOp`（`types/expression.ts`，23 种）—— 状态层落数据 vs 上层转发：
...
| `loot` | DropTableId | → LootSystem |
| `triggerStory` | StoryId（`owner` 决定沙盒游标） | → StoryService |
| `travelToArea` | AreaId（`notice` 控制展示条目） | → Story/移动 |
| `setTheme` | —（`value: ThemeEffectValue`） | → ColorSystem |
...
`ThemeEffectValue.scope`（`types/expression.ts`）：`ephemeral`（临时演出，默认）/ `area`（场景）/ `student`（学生）；`area`/`student` 需 `entityKey`（`area:<id>` / `variant:<id>`）。

## 5. 触发与持续效果

`TriggerEventDef.kind`（`types/trigger.ts`，7 种）—— Trigger「何时检查」，均可带可选过滤 id：
...
`AffectorState`（`types/trigger.ts`）：`Latent`（挂载未激活）→ `Active`（激活沿执行 `effects`）→ `Removed`（卸载）。

## 6. Extra 数据树（类 NBT）

`ExtraValue.t`（`types/extra.ts`，6 种）：`int` / `float` / `str` / `bool` / `list`（`ExtraValue[]`，允许异构）/ `dict`（`Record<string, ExtraValue>`，即 NBT Compound）。`ExtraCompound` = `dict` 别名（Def 的 `extra` 字段用）。`ExtraPath` = `/` 分隔字符串（如 `meta/rank`、`inv/0/name`；段不得为空，dict key 禁含 `/`）。三层合并视图：全局 → per-Init → 数据包常量表。

## 7. 世界实体功能

`SpotFunctionalityDef.kind`（`types/world.ts`，4 种）：
...
`InitPurchaseError`（`types/world.ts`）：`NotFound` / `AlreadyUnlocked` / `InsufficientResource`。

## 8. 揭示与可达性阶梯

`RevealStage`（`types/reveal.ts`，7 级，L0→L6）：`invisible` → `presence` → `partial` → `known` → `utility` → `purchaseable` → `owned`。

`RevealTarget`（`types/reveal.ts`，5 种）：`existence`（实体是否出现）/ `name` / `condition` / `utility` / `unlock`（实际解锁，engine 直接消费；其余仅信息揭示）。

`AccessStage`（`types/reveal.ts`，5 阶段，自上而下收窄）：`hidden`（可见性层）→ `obfuscated`（遮挡 ???）→ `revealed`（完整展示）→ `accessible`（可进入/解锁/使用）→ `active`（运行时持续生效）。
...
**ItemDef**（`types/content.ts`）：`rarity` = `common`/`rare`/`epic`/`legendary`；`type` = `consumable`/`material`/`key`。

**EnhancementAttachment.kind**（`types/content.ts`，仅 UI 展示）：`area` / `init` / `global`。

**StoryEntryBase.completionStrategy**（`types/content.ts`）：`simple`（默认，用 `completionReward.first/repeat`）/ `conditional`（用 `conditionalRewards` 按序首个满足）。`StoryEntryDef.type`：`active`（主/支/羁绊入口）/ `passive`（随机闲聊）。

**ResourceDisplayDef.showWhen**（`types/common.ts`）：`always`（默认常显）/ `hasAmount`（仅持有量 > 0 显示）。
...
- `CompositionType`（ColorGroup 头像构成）：`solid`（单色）/ `gradient`（双色渐变）/ `duotone`（双色阶调）/ `pie`（饼图 3~6 色位）/ `radial`（径向渐变）；
...
`ChatTextKind`（`types/expression.ts`，`showChatText.kind`，无 talklet 时生效）：`default` / `kizuna` / `title` / `badge` / `note`。
`ChatTextFont`（`types/expression.ts`，`showChatText.style.font`）：`default` / `serif` / `sans` / `mono` / `handwritten`。
...
上一篇：[[docs-824/03e-id-reference-semantics]] · 返回：[[docs-824/03-data-structures]]
